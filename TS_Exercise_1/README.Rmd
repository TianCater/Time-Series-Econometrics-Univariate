...
The p-value = 0.1686 > 0.05 means we fail to reject the null. Therefore the residuals are independent at the 95% level and thus an AR(1) model provides a good model fit (by the measure of autocorrelation between residuals).

  3.1.2. ARCH LM test (Hetreoskedasticity)
...
The p-value = 0.003976 > 0.05 means we reject the null at the 5% significance level. That is, the residuals are not normally distributed.
 ...
 3.1.3. ARCH LM test (Hetreoskedasticity)
...
```
  ...
  For the y process, the AR(1) model has the lowest AIC and BIC values (205.09 and 215.16, respectively). Therefore, preliminary we select this model and conduct further tests for adequacy. 
...
Require the residuals to be white noise and normally distributed. Test for autocorrelation (Ljung Box test) and normality (Jarque-Bera test). 


Visually, The corelogram (ACF) of the residuals does indeed look like a realization of white noise and a normal distribution Finally, we perform the Ljung-Box test for 20 lags to confirm this
...
The p-value=0.5044 > 0.05 means we fail to reject the null. Therefore the residuals are independent at the 95% level and thus an AR(1) model provides a good model fit (by the measure of autocorrelation between residuals).
