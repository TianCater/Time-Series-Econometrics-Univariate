...
The test determines whether or not errors are iid (i.e. white noise) or whether there is something more behind them; whether or not the autocorrelation for the errors or residuals are non zero. Essentially, it is a test of lack of fit: if the autocorrelation of the residuals are very small, we say that the model doesn’t show ‘significant lack of fit’.
...
When the Ljung Box test is applied to the residuals of an ARIMA model, the degrees of freedom  must be equal to m-b, where where m is the time lag, and b the number of estimated parameters in the ARIMA(p,q) model. Thankfully, R selects the correct df by default. 
...
The p-value = 0.003976 < 0.05 means we reject the null hypothesis at the 5% significance level. That is, the residuals are not white noise. 
  ...
  3.2. Parsimonious 
       The first requirement is that the estimates be statistically significant. Then select the fitted model with the lowest AIC and BIC values, this model is the then         the most parsimonious.
4. Conclude with the estimated results of the BULSM.
